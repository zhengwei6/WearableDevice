...
#include <EEPROM.h>                              
#include <SoftwareSerial.h>                          // For 藍芽
...
void loop() {
    ...
    // otherwise, check for DMP data ready interrupt (this should happen frequently)
        ...
        // display real acceleration, adjusted to remove gravity
          ...
          mpu.dmpGetLinearAccel(&aaReal, &aa, &gravity);
          String returnString;
          returnString = String(time) + "," + String(float(aaReal.x)*ta) + "," + String(float(aaReal.y)*ta) + "," + String(float(aaReal.z)*ta) + ",";
          BTSerial.print(returnString);
          mpu.dmpGetGyro(&gyro,fifoBuffer);
          returnString = String(float(gyro.x)*tg) + "," + String(float(gyro.y)*tg) + "," + String(float(gyro.z)*tg) + ",";
          BTSerial.print(returnString);
          mpu.dmpGetQuaternion(&q, fifoBuffer);
          mpu.dmpGetGravity(&gravity, &q);
          mpu.dmpGetYawPitchRoll(ypr, &q, &gravity);
          returnString = String(ypr[0] * 180/M_PI) + "," + String(ypr[1] * 180/M_PI) + "," + String(ypr[2] * 180/M_PI);
          BTSerial.println(returnString);
         #endif
    }
    }
}
